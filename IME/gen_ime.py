...
# prefix index
# ---------------------------------------------------------------------------
def _build_syllable_set(word_path):
    """Build a set of known pinyin syllables from single-char entries."""
    syls = set()
    with open(word_path, encoding="utf-8") as f:
        for line in f:
            ...
            code = parts[0].lower()
            if not (code.isascii() and code.isalpha() and 1 <= len(code) <= 6):
                continue
            if any(len(e) == 1 for e in parts[1:]):
                syls.add(code)
    return syls


def _split_pinyin(code, syls):
    """Greedy longest-match split of a pinyin string into known syllables."""
    if not code: return []
    for i in range(min(6, len(code)), 0, -1):
        prefix = code[:i]
        if prefix in syls:
            rest = _split_pinyin(code[i:], syls)
            if rest is not None:
                return [prefix] + rest
    return None


def _shorthand_parts(syllables):
    """Generate shorthand initials: 'zh','ch','sh' stay two-letter; others
    take first letter. e.g. ['zhong','guo','ren'] -> 'zhgr'."""
    out = []
    for s in syllables:
        if s[:2] in ('zh', 'ch', 'sh'):
            out.append(s[:2])
...
def _shorthand_parts_fuzzy(syllables):
    """Generate fuzzy shorthand initials: zh/ch/sh are treated as single-letter
    z/c/s. e.g. ['zhong','guo','ren'] -> 'zgr' (matches 'zhongguoren').
    This allows users to type 'zg' to match '中国' (zhōng guó)."""
    out = []
    for s in syllables:
        if s[:2] in ('zh', 'ch', 'sh'):
            out.append(s[0])  # zh->z, ch->c, sh->s
        else:
            out.append(s[0])
    return ''.join(out)
...
def all_gb2312(s, gb2312_set):
    """Check if all characters in string s are within GB2312 range."""
    for c in s:
...
def load_words(path, gb2312_set):
    """Yield (code, word_str, rank) from pinyin-utf.txt.
    Yields both single chars and multi-char words within GB2312 range.
    Complete pinyin codes only - shorthand matching is done at runtime."""
    syls = _build_syllable_set(path)
    print(f"pinyin syllables for shorthand: {len(syls)}", file=sys.stderr)
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            if len(parts) < 2: continue
            code = parts[0].lower()
            if not (code.isascii() and code.isalpha() and 1 <= len(code) <= 17):
                continue
            rank = 0
            for e in parts[1:]:
                # Yield both single chars (len 1) and words (len >= 2)
                # Filter: must be within GB2312 range
...
# GB2312 codepoint set (lazy init) for filtering the dictionary to characters
# the on-device font can actually render.
_gb2312_set = None


def _gb2312_codepoints():
    """Return a set of all GB2312 Unicode codepoints (incl. ASCII)."""
    global _gb2312_set
    if _gb2312_set is not None:
        return _gb2312_set
    _gb2312_set = set()
    # GB2312 rows 1..87 (0xA1..0xF7), cols 1..94 (0xA1..0xFE)
    for i in range(0xA1, 0xF8):
        for j in range(0xA1, 0xFF):
            try:
                _gb2312_set.add(ord(bytes([i, j]).decode("gb2312")))
            except (UnicodeDecodeError, LookupError):
                pass
    # also keep ASCII
    for c in range(0x20, 0x7F):
        _gb2312_set.add(c)
    # CJK punctuation blocks (common in dictionaries)
    for c in range(0x3000, 0x3040):
        _gb2312_set.add(c)
    for c in range(0xFF00, 0xFF60):
        _gb2312_set.add(c)
    return _gb2312_set


def is_in_font(s):
    """True if the character can be rendered by the GB2312 on-device font."""
    return len(s) == 1 and ord(s) in _gb2312_codepoints()
...
def load_pinyin(path, shuangpin=False):
    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            # single hanzi + single syllable + renderable on-device font only
            if len(char) != 1 or " " in syl or not is_in_font(char):
                continue
...
def main():
    ...
    ap.add_argument("--word-src", default="",
                    help="optional pinyin-utf.txt for multi-char word support")
    ...
    # ---- word section (optional) -------------------------------------------
    word_count = 0
    word_data_start = len(out)
    gb = _gb2312_codepoints()  # used for both words and predictions
    if args.word_src and os.path.exists(args.word_src):
        # collect words: dict code -> [(word, rank)]
        wdict = {}
        for code, word, rank in load_words(args.word_src, gb):
            if code not in wdict:
                wdict[code] = []
            wdict[code].append((word, rank))
        # sort by code asc
        sorted_codes = sorted(wdict.keys())

        # Build prefix index: for each two-letter prefix, store the byte
        # offset into wordData where groups with that prefix begin.
        word_index = [0] * INDEX_ENTRIES  # will be filled after we know offsets

        # First pass: build the word data and record group offsets.
        word_data = bytearray()
        code_offsets = {}  # code -> offset in word_data
        for code in sorted_codes:
            code_offsets[code] = len(word_data)
            # sort words within code by rank
            entries = sorted(wdict[code], key=lambda x: x[1])
            if len(entries) > 255:
                entries = entries[:255]  # cap for uint8 count field
            cb = code.encode("ascii")
            word_data.append(len(cb))
            word_data += cb
            word_data.append(len(entries))
            for word, _rank in entries:
                wb = word.encode("utf-8")
                word_data.append(len(wb))
                word_data += wb

        # Now compute the prefix index: for each 2-letter prefix k,
        # find the first code in sorted_codes whose prefix >= k, and
        # use its offset.
        WORD_INDEX_ENTRIES = 677
        # sentinel at the end
        for k in range(WORD_INDEX_ENTRIES):
            # target prefix as 2-char string
            c0 = chr(ord('a') + k // 26) if k < 676 else 'z'
            c1 = chr(ord('a') + k % 26) if k < 676 else 'z'
            target = c0 + c1 if k < 676 else "zz" + chr(ord('z') + 1)
            # find first code >= target
            idx = len(word_data)  # default: sentinel (past end)
            for code in sorted_codes:
                if code[:2] >= target:
                    idx = code_offsets[code]
                    break
            word_index[k] = idx

        word_count = sum(len(v) for v in wdict.values())

        out += struct.pack("<I", word_count)
        out += struct.pack("<%dI" % WORD_INDEX_ENTRIES, *word_index)
        out += word_data
        print(f"words        : {word_count}")
        print(f"word bytes   : {len(word_data)}  ({len(word_data)/1024:.1f} KiB)")

        out += struct.pack("<I", 0)  # predCount = 0 (disabled)
    else:
        out += struct.pack("<I", 0)  # wordCount = 0
        out += struct.pack("<I", 0)  # predCount = 0
