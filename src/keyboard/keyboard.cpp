...
bool keyboard_ime_filter(int key, bool pressed)
{
  ...
  // Pass through when the IME is off, while we are emitting a commit,
  // or when not in the word processor (so menu keys like 'b' for Back work).
  if (!ime.active() || ime_emitting
      || status()["screen"].as<int>() != WORDPROCESSOR)
    return false;
  ...
  // Chinese punctuation conversion when IME is active.  Common ASCII
  // punctuation keys produce their full-width / CJK equivalents so the
  // user doesn't need to switch modes for proper Chinese punctuation.
  if (key >= 32 && key < 127 && !ime.predicting())
  {
    const char *cn = NULL;
    switch (key)
    {
    case '.': cn = "\xE3\x80\x82"; break; // 。
    case ',': cn = "\xEF\xBC\x8C"; break; // ，
    case ';': cn = "\xEF\xBC\x9B"; break; // ；
    case ':': cn = "\xEF\xBC\x9A"; break; // ：
    case '?': cn = "\xEF\xBC\x9F"; break; // ？
    case '!': cn = "\xEF\xBC\x81"; break; // ！
    case '(': cn = "\xEF\xBC\x88"; break; // （
    case ')': cn = "\xEF\xBC\x89"; break; // ）
    case '<': cn = "\xE3\x80\x8A"; break; // 《
    case '>': cn = "\xE3\x80\x8B"; break; // 》
    case '"': cn = "\xE2\x80\x9C"; break; // "
    }
    if (cn)
    {
      consumed[key] = true;
