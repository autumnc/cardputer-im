...
public:
    ...
    // predictions until the user types a letter or cancels.
    void beginPredict(const String &text);
    void endPredict() { _predicting = false; _predChar = ""; }
    bool predicting() const { return _predicting; }

    // User dictionary management
    void removeUserWord(const String &code, const String &word);
    void clearUserDict();
    ...
    // Dynamic page size: set based on screen width so all candidates are
    // always visible on one page (no invisible spill-over).
...
private:
    ...
    std::vector<uint32_t> _wordIndex;   // 677 prefix-index offsets into _wordData
    const uint8_t *_wordData = nullptr; // pointer into _blob where word data starts
    size_t _wordDataSize = 0;

    // ---- prediction section (联想, optional) ----
    uint32_t _predCount = 0;
    const uint8_t *_predData = nullptr;
    size_t _predDataSize = 0;
    bool _predicting = false;      // true while showing predictions
    String _predChar;              // last committed text for prediction lookup
    int _partialStart = 0;         // index in _all where 逐字候选项开始
    int _maxMatchLen = 0;          // longest code that matched (for remainder calc)
    String _prefix;                // selected hanzi before remaining pinyin
    String _remainder;             // unmatched code suffix for 逐字匹配
    String _codeOrig;              // original typed code before pre-edit modifies it
    // User dictionary: loaded from SPIFFS at init, saved when new words added
    struct UserEntry { String code; String word; int count; };
    std::vector<UserEntry> _userWords;
    bool _userDirty = false;
    void loadUserDict();
    void saveUserDict();
    void addUserWord(const String &code, const String &word);
    void bumpFrequency(const String &code, const String &word);
    // 删除模式: v键激活,只匹配用户词库,选择后删除
    bool _deleteMode = false;
    // 两分拆字 (embedded in flash)
    bool _lfMode = false;
    const uint8_t *_lfBlob = nullptr;
    uint32_t _lfCount = 0;
    size_t _lfRecordBase = 0;
    std::vector<uint16_t> _lfIndex; // 4493 records fits in uint16_t, saves 1.3 KB
    void loadLfDict();
    void searchLfWindow(const char *code, int len, uint32_t &lo, uint32_t &hi);
    bool readLfCode(uint16_t i, char out[7]);
