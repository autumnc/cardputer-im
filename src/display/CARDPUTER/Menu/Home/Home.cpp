...
static lgfx::U8g2font menuFont(u8g2_font_terminus28_tf);
    ...
    const int lineH = M5Cardputer.Display.fontHeight() + 2;
    const int visible_lines = (M5Cardputer.Display.height() - 20) / lineH;
...
void Home_keyboard(char key)
{
    ...
    else if (key == 27 || key == '\n' || key == MENU)
    {
        ...
        {
            _debug("Flomo Selected\n");
            app["menu"]["state"] = MENU_FLOMO;
        }
    }
}
