...
#include "display/CARDPUTER/Menu/Sync/Sync.h"
#endif
...
void setup()
{
    ...
    if (app_ready())
    {
        ...
        xTaskCreatePinnedToCore(
            ...
            "SecondaryCore", // Name
            20480,           // Stack size (20KB for WiFi init + SD sync + safety margin)
            NULL,            // Parameters
            1,               // Priority
            NULL,            // Task handle
            1                // Core 1
...
#if defined(CARDPUTER_ADV)
    // Process sync request from menu (WiFi init needs clean stack on Core 0)
    Sync_process_in_loop();
#endif
}
